...
{
	current_pressure = wl_fixed_to_double(pressure);
}

static int
tablet_motion_handler(struct widget *widget, struct tablet *tablet, float x,
		      float y, uint32_t time, void *data)
{
	current_x = x;
	current_y = y;
...
static void
tablet_down_handler(struct widget *widget, struct tablet *tablet, uint32_t time,
		    void *data)
{
	tablet_is_down = true;
}
...
static void
tablet_up_handler(struct widget *widget, struct tablet *tablet, uint32_t time,
		  void *data)
{
...
init_globals()
{
	...
	widget_set_tablet_motion_handler(widget, tablet_motion_handler);
	widget_set_tablet_down_handler(widget, tablet_down_handler);
	widget_set_tablet_up_handler(widget, tablet_up_handler);
	widget_set_tablet_pressure_handler(widget, pressure_handler);
	widget_set_tablet_proximity_in_handler(widget, proximity_in_handler);
