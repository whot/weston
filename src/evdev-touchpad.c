...
get_touchpad_model(struct evdev_device *device)
{
	...
	id_vendor = libevdev_get_id_vendor(device->dev);
	id_product = libevdev_get_id_product(device->dev);

	for (i = 0; i < ARRAY_LENGTH(touchpad_spec_table); i++)
		if (touchpad_spec_table[i].vendor == id_vendor &&
		    (!touchpad_spec_table[i].product ||
		     touchpad_spec_table[i].product == id_product))
	      ...
	      struct evdev_device *device)
{
	...
	has_buttonpad = libevdev_has_property(device->dev, INPUT_PROP_BUTTONPAD);

	/* Configure pressure */
	if (libevdev_has_event_code(device->dev, EV_ABS, ABS_PRESSURE)) {
		configure_touchpad_pressure(touchpad,
					    libevdev_get_abs_minimum(device->dev, ABS_PRESSURE),
					    libevdev_get_abs_maximum(device->dev, ABS_PRESSURE));
	}
